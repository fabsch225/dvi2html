...
// Compute the pointer to the word element index from the base.  A word element has a size of 32-bit.
function word_ptr(base: number, index: number) {
    return base + 4 * index;
}
...
class TFMParser {
    ...
    read_unsigned_byte1(p?: number) {
        if (p) this.position = p;
        const result = this.stream.readUInt8(this.position);
        this.position = this.position + 1;
        return result;
    }

    read_unsigned_byte2(p?: number) {
        if (p) this.position = p;
        const result = this.stream.readUInt16BE(this.position);
        this.position = this.position + 2;
        return result;
    }

    read_unsigned_byte4(p?: number) {
        if (p) this.position = p;
    ...
    // Return the four numbers in table at index.
    read_four_byte_numbers_in_table(table: number, index: number) {
    ...
    // Return the extensible recipe, four numbers, at index.
    ...
    read_extensible_recipe(index: number) {
    ...
    read_fix_word_in_table(table: number, position: number) {
        return this.read_fix_word(this.position_in_table(table, position));
    }

    // BADBAD
    read_bcpl(position?: number) {
        if (position) this.position = position;
        const length = this.read_unsigned_byte1();
        const result = this.stream.subarray(this.position, this.position + length).toString('ascii');
        this.position += length;
        return result;
    }

    seek_to_table(table: number, position?: number) {
        if (position) this.seek(this.position_in_table(table, position));
        else this.seek(this.table_pointers[table]);
    }

    position_in_table(table: number, index: number) {
    ...
    */
    read_lengths() {
        ...
        const header_length = this.read_unsigned_byte2();
        this.smallest_character_code = this.read_unsigned_byte2();
        this.largest_character_code = this.read_unsigned_byte2();

        // 18 is the header data minimum length.
        this.table_lengths[tables.header] = Math.max(18, header_length);
        ...
        // Sanity check
        const length = this.position_in_table(tables.font_parameter, this.table_lengths[tables.font_parameter]);
        if (length != word_ptr(0, this.entire_file_length)) throw new Error('Bad TFM file');
    ...
    read_header() {
        ...
        // Read header[0 ... 1]
        const checksum = this.read_unsigned_byte4();
        const designSize = this.read_fix_word();
        ...
        if (position < character_info_table_position) {
            this.read_unsigned_byte1(position); // Skip the seven bit safe flag.
            this.read_unsigned_byte2();
            this.read_unsigned_byte1(); // Skip the face.
        }
    ...
    read_font_parameters() {
        ...
        if (this.tfm.character_coding_scheme == 'TeX math italic') {
            ...
            //throw new Error('undocumented character coding scheme');
        } else {
            // Read the seven fix word parameters
            this.tfm.set_font_parameters([...Array(7).keys()].map(() => this.read_fix_word()));
        }

        if (this.tfm.character_coding_scheme == 'TeX math symbols') {
            // Read the additional 15 fix word parameters
            this.tfm.set_math_symbols_parameters([...Array(15).keys()].map(() => this.read_fix_word()));
        }

        if (
            this.tfm.character_coding_scheme == 'TeX math extension' ||
            this.tfm.character_coding_scheme == 'euler substitutions only'
        ) {
            // Read the additional 6 fix word parameters
            this.tfm.set_math_extension_parameters([...Array(6).keys()].map(() => this.read_fix_word()));
        }
    }
    ...
    read_lig_kern_programs() {
        ...
        const first_skip_byte = this.read_unsigned_byte1();
        let next_char = this.read_unsigned_byte1();
        let op_byte = this.read_unsigned_byte1();
        let remainder = this.read_unsigned_byte1();

        if (first_skip_byte == 255) throw new Error('Font has right boundary char');

        this.seek_to_table(tables.lig_kern, this.table_lengths[tables.lig_kern] - 1);
        const last_skip_byte = this.read_unsigned_byte1();
        next_char = this.read_unsigned_byte1();
        op_byte = this.read_unsigned_byte1();
        remainder = this.read_unsigned_byte1();

        if (last_skip_byte == 255) throw new Error('Font has left boundary char program');
        ...
        for (let i = 0; i < this.table_lengths[tables.lig_kern]; ++i) {
            ...
            if (op_byte >= KERN_OPCODE) {
                ...
                new Tfm.TfmKern(
                    ...
                    stop,
                    next_char,
                    this.read_fix_word_in_table(tables.kern, 256 * (op_byte - KERN_OPCODE) + remainder)
                );
            } else {
                // Ligature step
                // Fixme: self registration?
                new Tfm.TfmLigature(
                    ...
                    stop,
                    next_char,
                    remainder, // ligature character code
                    op_byte >> 2, // number of characters to pass over
                    (op_byte & 0x02) == 0, // current character is deleted?
                    (op_byte & 0x01) == 0 // next character is deleted?
                );
            }
    ...
    process_char(c: number) {
        const info = this.read_char_info(c);
        const width_index = info.width_index;
        const height_index = info.height_index;
        const depth_index = info.depth_index;
        const italic_index = info.italic_index;
        const tag = info.tag;
        const remainder = info.remainder;

        // Get the parameters in the corresponding tables
        let width = 0;
        if (width_index != 0) width = this.read_fix_word_in_table(tables.width, width_index);
    ...
    read_char_info(c: number) {
        const index = c - this.smallest_character_code;
        const bytes: number[] = [];
